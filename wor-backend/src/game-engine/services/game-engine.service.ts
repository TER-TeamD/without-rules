...
import {Game, GameDocument, InPlayerGameProperty, Player,} from '../schema/game.schema';
...
export class GameEngineService implements OnModuleInit {
    ...
    //   }
    //
    //   await this.giveCardsToTableAtTheBeginningOfGame(
    //     game._id,
    //     game.in_game_property.stacks,
    //   );
    // }
    //
    // public async playerPlayedACard(
    //   idPlayer: string,
    //   body: PlayedCardDto,
    // ): Promise<StatusDto> {
    //   const game: Game = await this.gameModel.findOne({ 'players.id': idPlayer });
    ...
    //   );
    //
    //   await this.sendNextRound(game._id);
    // }
    //
