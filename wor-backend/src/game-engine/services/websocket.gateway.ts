...
import {Action, Card, Game, Player, Result, StackCard} from '../schema/game.schema';
...
export class WebsocketGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    public async tableStartGame(client: Socket, payload: any): Promise<void> {
        this.logger.log(`Start new game -> ID : ${JSON.stringify(  this.entitiesConnected[client.handshake.auth.id], )} : ${JSON.stringify(payload)}`,);
    ...
    public async playerPlayedCard(client: Socket, payload: any): Promise<void> {
        ...
        this.logger.log( `Player played card -> ID : ${JSON.stringify(this.entitiesConnected[client.handshake.auth.id], )} : ${JSON.stringify(payload)} `,);

        await this.gameEngineService.playerPlayedCard(message.player_id, message.card_value);
    }
    ...
    public async tableAllPlayerPlayed(client: Socket, payload: any): Promise<void> {
        this.logger.log(`Table all player played -> ID : ${JSON.stringify(  this.entitiesConnected[client.handshake.auth.id], )} : ${JSON.stringify(payload)}`,);

        await this.gameEngineService.tableAllPlayerPlayed();
    }

    @SubscribeMessage('TABLE_NEXT_ROUND_RESULT_ACTION')
    public async tableNextRoundResultAction(client: Socket, payload: any): Promise<void> {
        const message: { choosen_stack: number | null } = payload;
        this.logger.log(`Table all player played -> ID : ${JSON.stringify(  this.entitiesConnected[client.handshake.auth.id], )} : ${JSON.stringify(payload)}`,);

        await this.gameEngineService.tableNextRoundResultAction(message.choosen_stack);
    }
